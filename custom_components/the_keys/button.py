...
async def async_setup_entry(
    ...
    for device in coordinator.data:
        if isinstance(device, TheKeysLock):
            entities.append(TheKeysUnlockButton(device))
            entities.append(TheKeysCalibrateButton(device))
            entities.append(TheKeysSyncButton(device))

    async_add_entities(entities, update_before_add=False)


class TheKeysButtonEntity(TheKeysEntity, ButtonEntity):
    """Base class for TheKeys button entities."""

    def __init__(self, device: TheKeysLock, button_type: str):
...
class TheKeysUnlockButton(TheKeysButtonEntity):
    """Button to unlock The Keys lock."""

    def __init__(self, device: TheKeysLock):
        """Init unlock button."""
        super().__init__(device, "unlock")
    ...
    async def async_press(self) -> None:
        ...
        try:
            ...
            _LOGGER.info("Unlock command sent to %s", self._device.name)
...
class TheKeysCalibrateButton(TheKeysButtonEntity):
    ...
    def __init__(self, device: TheKeysLock):
        ...
        super().__init__(device, "calibrate")
        self._attr_icon = "mdi:tune"

    async def async_press(self) -> None:
        """Handle the button press."""
        try:
            # Check if calibrate method exists
